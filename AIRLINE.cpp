...
struct Airline {
	...
	redo:
		...
		cout << GREEN<< "\n\t\t\t\t\t\t\t\t| ENTER THE LOGIN ID: " << RESET;
		cin >> logdet.loginid;
		cout << GREEN << "\n\t\t\t\t\t\t\t\t| ENTER THE LOGIN PASSWORD: " << RESET;
		cin >> logdet.loginpassword;
		if (logdet.loginid != "a" || logdet.loginpassword != "1")
		{
	...
	void displaytickets(int i,string s)
	{
		...
		// Top border
		cout << MAGENTA << "\t\t\t\t\t\t\t"  << setfill('=') << setw(width) << "=" << endl;
		cout  << setfill(' '); // reset fill character

		// Header
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setw((width - 12) / 2) << "" << "BOARDING TICKET"  << endl;
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setfill('=') << setw(width) << "=" << endl;
		cout  << setfill(' '); // reset fill character

		// Left section details
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(12) << left << "PASSENGER" << ": " << passdett.passengerfirstname[i]<<" "<< passdett.passengerlastname[i] << endl;
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(12) << left << "TICKET ID" << ": " << ticdet.ticketIDs[i] << endl;
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(12) << left << "FLIGHT ID" << ": " << fnit.flightid[i] << endl;


		// Right section details
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << left << "DESTINATION" << ": " << "Islamabad" << " ---> " << flightN[choice - 1] << endl;
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << left << "SEAT" << ": " << s  << endl;
		cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << left << "DATE" << ": " <<  randomDate.tm_year + 1900 << "-" << randomDate.tm_mon + 1 << "-" << randomDate.tm_mday << endl;
		if (choice == 1) {
			cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << "Departure Time : 09 : 30 AM        Arrival Time : 10 : 30 AM " << RESET;
		}
		else if (choice == 2) {
			cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << "Departure Time : 10 : 45 AM        Arrival Time : 12 : 30 PM " << RESET;
		}
		else if (choice == 3) {
			cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << "Departure Time : 01 : 00 PM        Arrival Time : 01 : 45 PM " << RESET;
		}
		else if (choice == 4) {
			cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << "Departure Time : 02 : 00 PM        Arrival Time : 03 : 00 PM " << RESET;
		}
		else if (choice == 5) {
			cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << "Departure Time : 03 : 20 PM        Arrival Time : 04 : 20 PM " << RESET;
		}
		else if (choice == 6) {
			cout << MAGENTA << "\t\t\t\t\t\t\t" << setw(15) << "Departure Time : 04 : 30 PM        Arrival Time : 05 : 30 PM " << RESET;
		}
		// Bottom border
...
void displayboard() {
	...
	cout << CYAN << "\t\t\t\t\t\t\t\t|8. EXIT OR ENTER NEXT   |" << RESET << endl;
...
void initializeSeats(char seats[ROWS][COLUMNS]) {
	...
	for (int i = 0; i < ROWS; ++i) {
		for (int j = 0; j < COLUMNS; ++j) {
			seats[i][j] = (rand() % 2 == 0) ? '_' : 'X';  // Randomly assign 'O' or 'X'
		}
	}
}
...
reenterpassengernumber:
	cout << GREEN << "\n\t\t\t\t\t\t\t\t| Enter number of passengers: " << RESET;
	...
	for (int i = 0; i < num; i++)
	{
		do
		{
			...
			case 1:
			{
				airline.putdetails(i); // Get passenger details
				airline.displaypassengerdetails(i);
			...
			case 4:
			{
				airline.flights(i); // Select flight and display confirmation
				airline.addtickets(i); // Add ticket details
				airline.displaytickets(i,nseat); // Display ticket details
				break;
			}
			case 2:
			{
				airline.luggagedetails(i);  // Get luggage details
				break;
			}
			case 3:
			{
			...
			case 5:
			{
				airline.details(); // Display noticeboard details
				break;
			}
			case 6:
			{
				airline.check_in(); // Perform check-in
				airline.boardingpassgeneration(i); // Generate boarding pass
				break;
			}
			case 7:
			{
				ofstream file("Airline management proj.txt", ios::app);
				{
					...
					airline.writepastofile(file, i);
					airline.writeflighttofile(file, i);
					airline.writelugtofile(file, i);
					writeseat_reservation_tofile(file);
					airline.writeticktofile(file, i);
					airline.writenottofile(file);
					airline.writeboard_tofile(file, i);
					file << endl;
				}
				file.close();
				break;
